    ...
    with get_db_connect() as conn:
        ...
        if flask.request.method == 'POST':
            ...
            if tool != 'pass_find':
                ...
                if email_data:
                    curs.execute(db_change("select html from html_filter where html = ? and kind = 'email'"),
                                 [email_data.group(1)])
            ...
            sql_d = curs.fetchall()
            i_text = (html.escape(sql_d[0][0]) + '\n\nKey : ' + flask.session['c_key']) if sql_d and sql_d[0][
                0] != '' else ('Key : ' + flask.session['c_key'])
            ...
            if tool == 'pass_find':
                ...
                return easy_minify(flask.render_template(skin_check(),
                                                         imp=[load_lang('password_search'), wiki_set(), wiki_custom(),
                ...
                return easy_minify(flask.render_template(skin_check(),
                                                         imp=[load_lang('email'), wiki_set(), wiki_custom(),
                                                              wiki_css([0, 0])],
                                                         data='''
